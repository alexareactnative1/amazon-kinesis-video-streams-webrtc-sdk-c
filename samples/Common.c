...
STATUS getSelectedIceCandidateInformation(PSampleStreamingSession pSampleStreamingSession) {
    ...
    DLOGD("Local Candidate IP Address: %d", rtcMetrics.rtcStatsObject.localIceCandidateStats.address);
    ...
    DLOGD("Remote Candidate relay protocol: %s", rtcMetrics.rtcStatsObject.remoteIceCandidateStats.relayProtocol);
    DLOGD("Remote Candidate Ice server source: %s", rtcMetrics.rtcStatsObject.remoteIceCandidateStats.url);
CleanUp:
    LEAVES();
    return retStatus;
}
