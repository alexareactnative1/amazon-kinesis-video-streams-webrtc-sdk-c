...
STATUS updateSelectedLocalRemoteCandidateStats(PIceAgent pIceAgent) {
    ...
    MEMCPY(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.address, pIceAgent->pDataSendingIceCandidatePair->local->ipAddress.address, SIZEOF(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.address));
    ...
    STRCPY(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.url, pIceAgent->iceServers[pIceAgent->pDataSendingIceCandidatePair->local->iceServerIndex].url);
    STRCPY(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.protocol, pIceAgent->rtcIceServerDiagnostics[pIceAgent->pDataSendingIceCandidatePair->local->iceServerIndex].protocol);
    if (pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.candidateType == ICE_CANDIDATE_TYPE_RELAYED) {
        switch (pIceAgent->pDataSendingIceCandidatePair->local->pTurnConnection->protocol) {
            ...
            case KVS_SOCKET_PROTOCOL_TCP:
                ...
                break;
            default:
                MEMSET(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.relayProtocol, 0, SIZEOF(pIceAgent->rtcSelectedLocalIceCandidateDiagnostics.relayProtocol));
        }
    }

    // Get remote candidate related stats
    MEMCPY(pIceAgent->rtcSelectedRemoteIceCandidateDiagnostics.address, pIceAgent->pDataSendingIceCandidatePair->remote->ipAddress.address, SIZEOF(pIceAgent->rtcSelectedRemoteIceCandidateDiagnostics.address));
    ...
    STRCPY(pIceAgent->rtcSelectedRemoteIceCandidateDiagnostics.protocol, pIceAgent->rtcIceServerDiagnostics[pIceAgent->pDataSendingIceCandidatePair->remote->iceServerIndex].protocol);
    // Reference: https://www.w3.org/TR/webrtc-stats/#icecandidate-dict*
    STRCPY(pIceAgent->rtcSelectedRemoteIceCandidateDiagnostics.relayProtocol, "N/A"); // By spec, this field is not populated for remote candidate
    STRCPY(pIceAgent->rtcSelectedRemoteIceCandidateDiagnostics.url, "N/A"); // By spec, this field is not populated for remote candidate
CleanUp:
    return retStatus;
}
